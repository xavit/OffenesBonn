<?php
    ...
    # Variablen f�r die Datenbankverbindung includieren
	...
	define("EZSQL_DB_PASSWORD", DB_PASSWD);	// <-- mysql db password
	...
	class db_class {
		...
		function db_class($dbuser, $dbpassword, $dbname, $dbhost)
		{
			...
			if ( ! $this->dbh )
			{
				$this->print_error("<h1>Keine Datenbank Verbindung</h1><p>Bitte starten Sie das Setup!</p>");
				die;
			}
		...
		function print_error($str = "")
		{
			...
			if ( $this->show_errors )
			{
				#debug::print_d(debug_backtrace());
				#echo debug_backtrace();
				// If there is an error then take note of it
				print "<blockquote><font face=arial size=2 color=ff0000>";
				print "<b>SQL/DB Error --</b> ";
				print "[<font color=000077>".htmlentities($str)."</font>]";
				print "</font></blockquote><br /><blockquote><font face=arial size=2 color=000077>".htmlentities($this->last_query)."</font></blockquote>";
			}
			else
			{
				return false;
      ...
      $this->filename = $filename = PAPOO_ABS_PFAD . '/dokumente/logs/';
       $erstellen = fopen( $this->filename . "sql_log.log", "a" );
        $filename2 = $this->filename . "sql_log.log";
        ...
        $this->last_query=str_replace("\t","",$this->last_query);
        $inhalt .= "; ".$this->last_query;
        $inhalt .= "\r";
