General
=======
mPDF 5.0 has improved font handling which makes it a lot easier to use any (TrueType) fonts you want,
...
Next it will look for a translation (if set):
...
If you wish to make this font available, you need the Truetype .ttf font files for each variant.
...
In mPDF 5.0 Arial, Helvetica, Times and Courier are treated like any other font.

Unlike mPDF <= 4.x the whole CSS font string is parsed e.g.
style="font-family:'Arial Unicode MS', 'Lucida Grande', Helvetica;"
mPDF 4.0 only took the first word of the first font i.e. Arial
mPDF 5.0 will match the whole of 'Arial Unicode MS', and if this is not found will look for 'Lucida Grande'
or Helvetica
...
Arial Unicode MS. Set $this->backupSubsFont = 'arialunicodems'; at the top of the config_fonts.php file
to use this for any characters not found in the specific font being used.
...
2) looks in the default font for the document
3) [If defined] looks in the font set by $this->backupSubsFont
...
you can define the 2 constants before including the mpdf.php script e.g.:
